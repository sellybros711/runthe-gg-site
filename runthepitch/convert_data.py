...
# Source data filename — UPDATE on every data swap (mirrors convertData.js).
SOURCE_JSON = 'world_cup_full_rosters_1966_2026_RERATED.json'
IN_PATH  = os.path.join(BASE, 'data', SOURCE_JSON)
...
def val_or_null(v):
    # pass a value through unchanged; blank/missing -> None (ballon_dor)
    if v is None: return None
...
def as_bool(v):
    ...
    return v in (1, 1.0, True, '1') or (isinstance(v, str) and v.strip().lower() == 'true')
...
def main():
    ...
    for row in rows:
        ...
        players.append({
            ...
            'career_goals': int_or_null(row.get('career_goals_at_tournament')),
            ...
            'ballon_dor': val_or_null(row.get('ballon_dor')),  # empty for now; carried through
            'is_icon': as_bool(row.get('is_icon')),            # 372 historical icons flagged
        })
    ...
    print('is_icon players:', sum(1 for p in players if p['is_icon']), '(expected 372)')
    # Spot-check overalls against the known RERATED values.
    for nm, yr, exp in [('Pelé',1970,99),('Maradona',1986,99),('Messi',2022,99),
                        ('Messi',2006,86),('Thierry Henry',2006,93),('Thierry Henry',1998,83)]:
        got = confirm(nm, yr)
        print(f'{nm} {yr} wc_overall:', got, f'(expected {exp})', 'OK' if got == exp else 'MISMATCH')
    ...
    JS_PATH = os.path.join(BASE, 'data', 'players_all.js')
    header = ('// Auto-generated JS wrapper of players_all.json — lets the game run when\n'
              '// opened directly via file:// (where fetch() is blocked). Regenerate from\n'
              '// players_all.json whenever the data changes.\n')
    with open(JS_PATH, 'w', encoding='utf-8') as f:
        f.write(header + 'window.PLAYERS_ALL = ' + json.dumps(players, ensure_ascii=False) + ';')
    print('Wrote', JS_PATH)
