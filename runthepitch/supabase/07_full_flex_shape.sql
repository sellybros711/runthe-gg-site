-- ============================================================================
-- 07_full_flex_shape.sql  —  Accept the new Full-draft flex-builder shapes
-- ============================================================================
-- The Full (11-player) draft replaced the fixed formation wheel with a flex
-- builder: 1 GK + floors DEF3 / MID2 / FWD2 + 3 FLEX (DEF/MID/FWD, never GK),
-- cap 5 per outfield position. The legal shapes are now:
--   3-5-2, 3-4-3, 3-3-4, 3-2-5, 4-4-2, 4-3-3, 4-2-4, 5-3-2, 5-2-3
--   → DEF 3-5, MID 2-5, FWD 2-5, outfield = 10.
--
-- The previous server check (03/04) required MID 3-5 and FWD 1-3, which would
-- REJECT 3-2-5 / 3-3-4 / 4-2-4. This widens the full check to match. Scoring is
-- unchanged (avg(wc_overall) + 0.15*#99, × leadership multiplier) and remains
-- formation-independent. Quick-draft validation is unchanged.
--
-- Run AFTER 04_store_lineup.sql (this only replaces the function body).
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_dtype = 'full' THEN
    -- flex builder: DEF 3-5, MID 2-5, FWD 2-5, outfield = 10
    IF n_def NOT BETWEEN 3 AND 5 OR n_mid NOT BETWEEN 2 AND 5
       OR n_fwd NOT BETWEEN 2 AND 5 OR (n_def + n_mid + n_fwd) <> 10 THEN
