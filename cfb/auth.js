...
(function () {
  ...
  /* ENDING THE ACCOUNT. The whole job is one RPC -- see supabase/65_delete_account.sql --
     ...
     ONE ACCOUNT, EVERY GAME, so this is character for character what the football game
     calls, and it ends the account there too. The copy on the button says so before it
     does anything. */
  ...
  /* THE SAME PURCHASES, WITH THE PAPERWORK ON THEM.
     ...
     ONE ACCOUNT, EVERY GAME, so this is character for character what the football
     game calls. A bundle bought from the football store writes the same four rows
     whichever door the buyer came through, and a receipt that only one of the two
     games can show is a receipt half the buyers cannot find.
