/* A stand-in for PostgREST, big enough to serve every request cfb/board.js makes,
 * backed by a real Postgres running the real supabase/62_cfb_leaderboard.sql.
 *
 *   node cfb/build/test/postgrest_stub.mjs [port] [database]
 *
 ...
 * reads a count out of, and the shape of the payload cfb_submit_run() receives. All
 * of that has to survive contact with something that parses it independently, and
 * the validator on the other side has to be the real one.
 *
 ...
 * request, anything else is a guest. That is enough to exercise attribution and
 * claiming without an auth server in the loop.
 */
import { createServer } from 'http';
import { execFileSync } from 'child_process';

const PORT = Number(process.argv[2] || 5555);
const DB = process.argv[3] || 'cfbtest';
...
createServer(async (req, res) => {
  ...
  /* The bearer stands in for the JWT. */
  ...
  try {
    if (req.method === 'POST' && url.pathname === '/rest/v1/rpc/cfb_submit_run') {
      ...
      const args = [
        num(p.p_regular_wins), num(p.p_national_rank), num(p.p_playoff_wins),
        p.p_bowl_won ? 'true' : 'false',
        num(p.p_point_diff), num(p.p_chemistry_pct), num(p.p_spend_musd),
        num(p.p_respins), num(p.p_sig_wins), num(p.p_best_win_rank),
        arr(p.p_picks), arr(p.p_slots),
    ...
    if (req.method === 'GET' && url.pathname === '/rest/v1/cfb_runs') {
      ...
      const rows = JSON.parse(asUser(
        ...
        `(select ${sel} from cfb_runs${w}${ord} limit ${lim}) t`
      ));

      const headers = {};
      if (wantCount) {
        const total = Number(asUser(`select count(*) from cfb_runs${w}`));
