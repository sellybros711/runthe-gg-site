...
| `test_arcade_ad.mjs` | **The removal of the arcade house ad, which is now all this file can test.** It used to drive the panel in `/assets/arcade-ad.js` through the homepage and check its promises (once a visit, never again once the box is ticked, never over an open sheet, the opt-out on screen at six widths). The homepage never loaded the panel, so it had been timing out on its first assertion and nothing after that line ran, including the college game's own removal guard: a suite that throws in its first section is no suite. The NFL game was the only surface that ever loaded it, and it has now been removed there too, by request. So this holds both removals. Two halves: the markup read directly (no script tag, and nothing left calling `askArcadeAd()` or `RTG_ARCADE_AD`), then a browser check that neither game raises the panel on arrival, after a long look, or when `?arcadead=1` demands it. The static half is the load-bearing one, because the panel never shows itself: a page that keeps the tag and drops the caller sits silent through every browser assertion. The browser half skips rather than fails when the server is not up. `assets/arcade-ad.js` is deliberately still in the repo and loaded by nothing; the rules it used to be held to are in this file's history and should come back with it if it is ever wanted again. |
...
| `test_delete_button.mjs` | The way out of an account, in BOTH games from one file, because it is one account and one deletion and a check that ran on one would let the two drift. Delete my account was a full-width filled red button, the only filled control on a page whose other three are ghosts, which made ending your account the visual headline of managing it. Three things are held: that it is SMALL at rest (not filled, smaller type, and something else is the widest control on the page), that it is still FINDABLE (visible, labelled, under its own heading, big enough to tap, and the resting note still says site-wide and permanent), and that it GROWS to full width and filled at the armed step, because the tap after that one ends the account. The middle group is the one that will rot: nothing but a test notices if a later tidy-up quietly hides it behind a menu. |
| `test_board_gate.mjs` | The leaderboard behind glass for a guest, and the offer standing on it. Four states, and only the first is obvious: a guest sees the shape and cannot read the names; a member sees it clean with nothing sold on it; **accounts offline sees it clean too**, because charging for a board in a browser that cannot create an account is a wall with no door in it; and an EMPTY board is not blurred at all, because a card promising a leaderboard over a panel saying nobody has played yet reads as a bug. It also measures what an eye gets wrong: that the count above the glass stays readable (it is the whole argument for signing in), and that the sign-in button clears the sticky footer rather than hiding behind it. The peek above the card was 150px, looked right, and put that button 39px behind the dock at 390x900. |
...
node cfb/build/test/test_board_gate.mjs
