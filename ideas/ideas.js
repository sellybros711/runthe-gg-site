...
(function () {
  ...
  function load() {
    ...
    fetch(q, { headers: headers() })
      ...
      .catch(function (e) {
        state.busy = false;
        /* NOTHING DEVELOPER FACING REACHES THE PAGE. This used to print the raw PostgREST
           message for anything it did not recognise, and to say "not set up on this project
           yet" when the tables were missing, which is a sentence for whoever is building the
           site and not for whoever is reading it. It also made the page look broken rather
           than unavailable, which is the difference between a visitor shrugging and an
           AdSense reviewer marking a page. The real message still goes to the console. */
        try { console.warn('Community Ideas:', e.message); } catch (_) {}
        state.err = 'Community Ideas is unavailable right now. Try again shortly.';
        paint();
      });
  }
