...
--
-- ----------------------------------------------------------------------------
-- THIS IS 84 FOR THE SECOND TIME, AND THE REASON IS THE INTERESTING PART
-- ----------------------------------------------------------------------------
-- 84_football_bye_threshold.sql moved the server's bye threshold from 15 to 16
-- to match engine.js, and wrote out exactly why. Read it: the reasoning has not
-- changed and is not repeated here.
--
-- Then 86, 94 and 97 each added a column to ps_submit_run, and each one rebuilt
-- the whole function by copying an OLDER definition and editing it. All three
-- carried PS_BYE_SEED_WINS = 15 back in with them. The threshold has now been
-- reverted three times by three separate commits, none of which was about
-- seeding, and none of which failed anything: this is a constant pasted into a
-- body nobody rereads.
--
-- WHAT IT DID TO PLAYERS, live and today. A 15 win regular season is a WILD CARD
-- to the client, so it plays the four round bracket. Win all four and the run is
-- a champion at 19-2. The server, reading 15 as a top seed with a bye, allows
-- three playoff wins at most and REFUSES the submit outright:
--
--     playoff wins must be 0..3 for a Top seed seed, got 4
--
-- So the best runs this game can produce, a 15-2 season that goes on to win the
-- title, are the exact runs it throws away. The player is told the leaderboard
-- could not be reached, which is not what happened.
--
-- The milder half is already recorded and is not repaired here: a 15 win season
-- that did NOT win it all was accepted with the server's own seed_label, so
-- there are rows on the board reading "Top seed" for a run the player watched
-- being seeded as a wild card, and 84's header explains how that also invents a
-- trophy at three playoff wins. Those rows are wrong and rewriting them means
-- deciding what a run nobody can replay actually was, so it is left alone rather
-- than guessed at.
--
-- scripts/check-submit-constants.mjs now fails the build when the newest
-- migration defining this function disagrees with engine.js, so a fourth revert
-- is caught by CI rather than by a player losing a title.
-- ----------------------------------------------------------------------------
...
declare
  ...
  PS_BYE_SEED_WINS   constant int := 16;   -- MATCHES engine.js BYE_SEED_WINS
