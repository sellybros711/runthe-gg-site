-- ============================================================================
-- 81_football_defense_submit.sql : the server accepts a One Stop run
-- ============================================================================
-- Safe to re-run: one CREATE OR REPLACE with the signature it already has, so
-- no overload is created and nothing has to be dropped.
--
-- RUN 80_football_defense_mode.sql FIRST if you have not. That one widens
-- ps_runs_run_mode_ck so a defense row can exist in the table. This one is the
-- other half: the table would have taken the row, and the function that writes
-- it refused to.
--
-- ----------------------------------------------------------------------------
-- WHY 80 WAS NOT ENOUGH, WHICH IS THE WHOLE POINT OF THIS FILE
-- ----------------------------------------------------------------------------
-- ps_submit_run is the only way a row reaches ps_runs, and it validates twice
-- more than the table does. Two of those checks reject a defensive season:
--
--   1. THE MODE LIST. `if v_mode not in ('free','club','era','trade')` raises
--      `unknown mode defense`. Widening the table's CHECK does not touch it.
--
--   2. THE SLOT LIST. A defensive roster fills DL DL LB DB DB FLEX. The
--      function accepted only QB RB WR TE FLEX and raised `unknown slot name`,
--      so even with the mode allowed the run died on its lineup. That one is
--      easy to miss: nothing about a mode's NAME suggests its LINEUP is
--      validated somewhere else.
--
-- ----------------------------------------------------------------------------
-- AND THE CLIENT HALF, WHICH IS IN THE SAME COMMIT
-- ----------------------------------------------------------------------------
-- Neither exception was firing, because board.js never sent 'defense'. Its
-- mode whitelist listed four modes and fell through to 'free', so a One Stop
-- season would have been recorded as free play, with defenders filed under
-- QB RB WR TE FLEX, on the board for a game it was not playing. That is the
-- Trade Machine bug this repo already fixed once (see 61), and it happened
-- again because the whitelist was written out at four separate call sites.
-- board.js now decides a mode name in one function.
--
-- ORDER: this file, then the deploy. Between the two, nothing changes: the
-- client is still sending 'free' until the new one lands.
-- ----------------------------------------------------------------------------
