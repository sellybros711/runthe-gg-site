-- CS483t: rank a completed CAREER on TODAY's + THIS WEEK's + all-time career boards, by total career NET
-- PROFIT (the sum of that career's posted season net) - mirroring runtour_season_rank (migration 57) but
-- AGGREGATED per career (user_id + career_id, matching runtour_career_board's grouping). A career's "when"
-- for the today/week windows is its COMPLETION time = the latest created_at among its seasons, so a career
-- finished today counts as "today". Window boundaries come from the client (the game's ET day start and the
-- UTC week block); both nullable so a 1-arg call resolves via defaults and returns just all-time.
