...
window.__S1 = {
  ...
  /* kind: plain | holder | loading | guest.  tier = where the free lane already sits.
     season = which pass season to pretend today falls in. */
  rig(o){
    ...
    if(o.seen){ LS.set(acctKey('bag_pass'), {s1end:1}); }
    S.screen='title'; S.overlay=null;
    return {signedIn:sbSignedIn(), season:passSeason().n, tier:passTierAt(passState().xp),
      holder:dailyPassActive()};
  },
...
/* pretend today falls in a different pass season, without moving the clock */
...
const run = async () => {
  ...
  ok('and it never opens again for that account', f1.overlay === null, f1);

  head('an account that saw it on another device');
  await rig({ kind: 'plain', seen: true });
  ok('does not see it here either', (await pending()) === false);
  const merged = await page.evaluate(() => mergePass({ s1end: 1 }, { claimed: 'S1' }));
  ok('because the flag survives a cloud merge from either side', merged.s1end === true, merged);
  const merged2 = await page.evaluate(() => mergePass({ claimed: 'S1' }, { s1end: 1 }));
  ok('...in both directions', merged2.s1end === true, merged2);
  const merged3 = await page.evaluate(() => mergePass({ claimed: 'S1' }, { claimed: 'S1' }));
  ok('and an account that has not seen it is not marked as having seen it', !merged3.s1end, merged3);
  ...
  head('screenshots');
  for (const [tag, o] of [['below20', { kind: 'plain', tier: 6 }], ['above20', { kind: 'plain', tier: 26 }]]) {
    await rig(o);
    await page.evaluate(() => window.__S1.paint());
  ...
  await page.evaluate(() => window.__S1.paint());
  await page.waitForTimeout(120);
  await page.screenshot({ path: `${SHOTS}/s1end-below20-desktop.png` });
