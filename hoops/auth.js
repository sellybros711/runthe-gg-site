/*
 * auth.js - accounts for Run The Floor.
 *
 ...
 * An account is a way to put your name on a run, never a gate in front of
 * playing one.
 *
...
(function () {
  ...
  const url = () => window.RTF_BOARD_URL || SB_URL;
  ...
  /* set_username() validates the format and the uniqueness server-side, so the
     client's own check is a courtesy and not the rule. rtf_rename_runs() then
  ...
  /* The run you finished as a guest, taken over now that you are signed in.
     ...
     enough to claim somebody else's run. The rule is in rtf_claim_run() and
     not here. */
  async function claim(runId) {
    if (!sb || !session || !runId) return false;
    try {
      const { data, error } = await sb.rpc('rtf_claim_run', { p_id: runId });
